...
DefNode(ShaderNode,     SH_NODE_TEX_IES,            def_sh_tex_ies,         "TEX_IES",            TexIES,           "IES Texture",       "Match real world lights with IES files, which store the directional intensity distribution of light sources")
...
DefNode(CompositorNode, CMP_NODE_ALPHAOVER,         def_cmp_alpha_over,     "ALPHAOVER",       AlphaOver,        "Alpha Over",          "Overlay a foreground image onto a background image")
DefNode(CompositorNode, CMP_NODE_ANTIALIASING,   def_cmp_antialiasing,      "ANTIALIASING",    AntiAliasing,     "Anti-Aliasing",       "Smooth away jagged edges")
DefNode(CompositorNode, CMP_NODE_BILATERALBLUR,  def_cmp_bilateral_blur,    "BILATERALBLUR",   Bilateralblur,    "Bilateral Blur",      "Adaptively blur image, while retaining sharp edges")
DefNode(CompositorNode, CMP_NODE_BLUR,              def_cmp_blur,           "BLUR",            Blur,             "Blur",                "Blur an image, using several blur modes")
DefNode(CompositorNode, CMP_NODE_BOKEHBLUR,      def_cmp_bokehblur,         "BOKEHBLUR",       BokehBlur,        "Bokeh Blur",          "Generate a bokeh type blur similar to Defocus. Unlike defocus an in-focus region is defined in the compositor")
DefNode(CompositorNode, CMP_NODE_BOKEHIMAGE,     def_cmp_bokehimage,        "BOKEHIMAGE",      BokehImage,       "Bokeh Image",         "Generate image with bokeh shape for use with the Bokeh Blur filter node")
DefNode(CompositorNode, CMP_NODE_BRIGHTCONTRAST, def_cmp_brightcontrast,    "BRIGHTCONTRAST",  BrightContrast,   "Brightness/Contrast", "Adjust brightness and contrast")
DefNode(CompositorNode, CMP_NODE_CHANNEL_MATTE,  def_cmp_channel_matte,     "CHANNEL_MATTE",   ChannelMatte,     "Channel Key",         "Create matte based on differences in color channels")
DefNode(CompositorNode, CMP_NODE_CHROMA_MATTE,   def_cmp_chroma_matte,      "CHROMA_MATTE",    ChromaMatte,      "Chroma Key",          "Create matte based on chroma values")
DefNode(CompositorNode, CMP_NODE_COLOR_MATTE,    def_cmp_color_matte,       "COLOR_MATTE",     ColorMatte,       "Color Key",           "Create matte using a given color, for green or blue screen footage")
DefNode(CompositorNode, CMP_NODE_COLOR_SPILL,    def_cmp_color_spill,       "COLOR_SPILL",     ColorSpill,       "Color Spill",         "Remove colors from a blue or green screen, by reducing one RGB channel compared to the others")
DefNode(CompositorNode, CMP_NODE_COLORBALANCE,   def_cmp_colorbalance,      "COLORBALANCE",    ColorBalance,     "Color Balance",       "Adjust color and values")
DefNode(CompositorNode, CMP_NODE_COLORCORRECTION,def_cmp_colorcorrection,   "COLORCORRECTION", ColorCorrection,  "Color Correction",    "Adjust the color of an image, separately in several tonal ranges (highlights, midtones and shadows)")
DefNode(CompositorNode, CMP_NODE_COMBHSVA_LEGACY,0,                         "COMBHSVA",        CombHSVA,         "Combine HSVA",        "Deprecated")
DefNode(CompositorNode, CMP_NODE_COMBINE_COLOR,  def_cmp_combsep_color,     "COMBINE_COLOR",   CombineColor,     "Combine Color",       "Combine an image from its composite color channels")
DefNode(CompositorNode, CMP_NODE_COMBINE_XYZ,    0,                         "COMBINE_XYZ",     CombineXYZ,       "Combine XYZ",         "Combine a vector from its individual components")
DefNode(CompositorNode, CMP_NODE_COMBRGBA_LEGACY,0,                         "COMBRGBA",        CombRGBA,         "Combine RGBA",        "Deprecated")
DefNode(CompositorNode, CMP_NODE_COMBYCCA_LEGACY,def_cmp_ycc,               "COMBYCCA",        CombYCCA,         "Combine YCbCrA",      "Deprecated")
DefNode(CompositorNode, CMP_NODE_COMBYUVA_LEGACY,0,                         "COMBYUVA",        CombYUVA,         "Combine YUVA",        "Deprecated")
DefNode(CompositorNode, CMP_NODE_COMPOSITE,         def_cmp_composite,      "COMPOSITE",       Composite,        "Composite",           "Final render output")
DefNode(CompositorNode, CMP_NODE_CONVERT_COLOR_SPACE,def_cmp_convert_color_space,"CONVERT_COLORSPACE",ConvertColorSpace,"Convert Colorspace","Convert between color spaces")
...
DefNode(CompositorNode, CMP_NODE_CROP,           def_cmp_crop,              "CROP",            Crop,             "Crop",                "Crops image to a smaller region, either making the cropped area transparent or resizing the image")
...
DefNode(CompositorNode, CMP_NODE_CURVE_RGB,         def_rgb_curve,          "CURVE_RGB",       CurveRGB,         "RGB Curves",          "Perform level adjustments on each color channel of an image")
DefNode(CompositorNode, CMP_NODE_CURVE_VEC,         def_vector_curve,       "CURVE_VEC",       CurveVec,         "Vector Curves",       "Map input vector components with curves")
DefNode(CompositorNode, CMP_NODE_DBLUR,          def_cmp_dblur,             "DBLUR",           DBlur,            "Directional Blur",    "Blur an image along a direction")
DefNode(CompositorNode, CMP_NODE_DEFOCUS,        def_cmp_defocus,           "DEFOCUS",         Defocus,          "Defocus",             "Apply depth of field in 2D, using a Z depth map or mask")
DefNode(CompositorNode, CMP_NODE_DENOISE,        def_cmp_denoise,           "DENOISE",         Denoise,          "Denoise",             "Denoise renders from Cycles and other ray tracing renderers")
...
DefNode(CompositorNode, CMP_NODE_DIFF_MATTE,     def_cmp_diff_matte,        "DIFF_MATTE",      DiffMatte,        "Difference Key",      "Produce a matte that isolates foreground content by comparing it with a reference background image")
DefNode(CompositorNode, CMP_NODE_DILATEERODE,    def_cmp_dilate_erode,      "DILATEERODE",     DilateErode,      "Dilate/Erode",        "Expand and shrink masks")
DefNode(CompositorNode, CMP_NODE_DISPLACE,       0,                         "DISPLACE",        Displace,         "Displace",            "Displace pixel position using an offset vector")
DefNode(CompositorNode, CMP_NODE_DIST_MATTE,     def_cmp_distance_matte,    "DISTANCE_MATTE",  DistanceMatte,    "Distance Key",        "Create matte based on 3D distance between colors")
DefNode(CompositorNode, CMP_NODE_DOUBLEEDGEMASK, def_cmp_double_edge_mask,  "DOUBLEEDGEMASK",  DoubleEdgeMask,   "Double Edge Mask",    "Create a gradient between two masks")
DefNode(CompositorNode, CMP_NODE_EXPOSURE,       0,                         "EXPOSURE",        Exposure,         "Exposure",            "Adjust brightness using a camera exposure parameter")
DefNode(CompositorNode, CMP_NODE_FILTER,            def_cmp_filter,         "FILTER",          Filter,           "Filter",              "Apply common image enhancement filters")
DefNode(CompositorNode, CMP_NODE_FLIP,           def_cmp_flip,              "FLIP",            Flip,             "Flip",                "Flip an image along a defined axis")
DefNode(CompositorNode, CMP_NODE_GAMMA,          0,                         "GAMMA",           Gamma,            "Gamma",               "Apply gamma correction")
DefNode(CompositorNode, CMP_NODE_GLARE,          def_cmp_glare,             "GLARE",           Glare,            "Glare",               "Add lens flares, fog and glows around bright parts of the image")
DefNode(CompositorNode, CMP_NODE_HUE_SAT,           0,                      "HUE_SAT",         HueSat,           "Hue/Saturation/Value","Apply a color transformation in the HSV color model")
DefNode(CompositorNode, CMP_NODE_HUECORRECT,     def_cmp_huecorrect,        "HUECORRECT",      HueCorrect,       "Hue Correct",         "Adjust hue, saturation, and value with a curve")
DefNode(CompositorNode, CMP_NODE_ID_MASK,        def_cmp_id_mask,           "ID_MASK",         IDMask,           "ID Mask",             "Create a matte from an object or material index pass")
DefNode(CompositorNode, CMP_NODE_IMAGE,             def_cmp_image,          "IMAGE",           Image,            "Image",               "Input image or movie file")
DefNode(CompositorNode, CMP_NODE_INPAINT,        def_cmp_inpaint,           "INPAINT",         Inpaint,          "Inpaint",             "Extend borders of an image into transparent or masked regions")
DefNode(CompositorNode, CMP_NODE_INVERT,         def_cmp_invert,            "INVERT",          Invert,           "Invert Color",        "Invert colors, producing a negative")
DefNode(CompositorNode, CMP_NODE_KEYING,         def_cmp_keying,            "KEYING",          Keying,           "Keying",              "Perform both chroma keying (to remove the backdrop) and despill (to correct color cast from the backdrop)")
DefNode(CompositorNode, CMP_NODE_KEYINGSCREEN,   def_cmp_keyingscreen,      "KEYINGSCREEN",    KeyingScreen,     "Keying Screen",       "Create plates for use as a color reference for keying nodes")
DefNode(CompositorNode, CMP_NODE_KUWAHARA,       def_cmp_kuwahara,          "KUWAHARA",        Kuwahara,         "Kuwahara",            "Apply smoothing filter that preserves edges, for stylized and painterly effects")
DefNode(CompositorNode, CMP_NODE_LENSDIST,       def_cmp_lensdist,          "LENSDIST",        Lensdist,         "Lens Distortion",     "Simulate distortion and dispersion from camera lenses")
DefNode(CompositorNode, CMP_NODE_LUMA_MATTE,     def_cmp_luma_matte,        "LUMA_MATTE",      LumaMatte,        "Luminance Key",       "Create a matte based on luminance (brightness) difference")
DefNode(CompositorNode, CMP_NODE_MAP_RANGE,         def_cmp_map_range,      "MAP_RANGE",       MapRange,         "Map Range",           "Map an input value range into a destination range")
DefNode(CompositorNode, CMP_NODE_MAP_UV,         def_cmp_map_uv,            "MAP_UV",          MapUV,            "Map UV",              "Map a texture using UV coordinates, to apply a texture to objects in compositing")
DefNode(CompositorNode, CMP_NODE_MAP_VALUE,         def_cmp_map_value,      "MAP_VALUE",       MapValue,         "Map Value",           "Scale, offset and clamp values")
DefNode(CompositorNode, CMP_NODE_MASK_BOX,       def_cmp_boxmask,           "BOXMASK",         BoxMask,          "Box Mask",            "Create rectangular mask suitable for use as a simple matte")
DefNode(CompositorNode, CMP_NODE_MASK_ELLIPSE,   def_cmp_ellipsemask,       "ELLIPSEMASK",     EllipseMask,      "Ellipse Mask",        "Create elliptical mask suitable for use as a simple matte or vignette mask")
DefNode(CompositorNode, CMP_NODE_MASK,           def_cmp_mask,              "MASK",            Mask,             "Mask",                "Input mask from a mask datablock, created in the image editor")
DefNode(CompositorNode, CMP_NODE_MATH,           def_math,                  "MATH",            Math,             "Math",                "Perform math operations")
DefNode(CompositorNode, CMP_NODE_MIX_RGB,           def_mix_rgb,            "MIX_RGB",         MixRGB,           "Mix",                 "Blend two images together using various blending modes")
DefNode(CompositorNode, CMP_NODE_MOVIECLIP,      def_cmp_movieclip,         "MOVIECLIP",       MovieClip,        "Movie Clip",          "Input image or movie from a movie clip datablock, typically used for motion tracking")
DefNode(CompositorNode, CMP_NODE_MOVIEDISTORTION,def_cmp_moviedistortion,   "MOVIEDISTORTION", MovieDistortion,  "Movie Distortion",    "Remove lens distortion from footage, using motion tracking camera lens settings")
DefNode(CompositorNode, CMP_NODE_NORMAL,            0,                      "NORMAL",          Normal,           "Normal",              "Generate a normal vector and a dot product")
...
DefNode(CompositorNode, CMP_NODE_RGBTOBW,           0,                      "RGBTOBW",         RGBToBW,          "RGB to BW",           "Convert RGB input into grayscale using luminance")
DefNode(CompositorNode, CMP_NODE_ROTATE,         def_cmp_rotate,            "ROTATE",          Rotate,           "Rotate",              "Rotate image by specified angle")
DefNode(CompositorNode, CMP_NODE_SCALE,          def_cmp_scale,             "SCALE",           Scale,            "Scale",               "Change the size of the image")
DefNode(CompositorNode, CMP_NODE_SCENE_TIME,     0,                         "SCENE_TIME",      SceneTime,        "Scene Time",          "Input the current scene time in seconds or frames")
DefNode(CompositorNode, CMP_NODE_SEPARATE_COLOR, def_cmp_combsep_color,     "SEPARATE_COLOR",  SeparateColor,    "Separate Color",      "Split an image into its composite color channels")
DefNode(CompositorNode, CMP_NODE_SEPARATE_XYZ,   0,                         "SEPARATE_XYZ",    SeparateXYZ,      "Separate XYZ",        "Split a vector into its individual components")
DefNode(CompositorNode, CMP_NODE_SEPHSVA_LEGACY,    0,                      "SEPHSVA",         SepHSVA,          "Separate HSVA",       "Deprecated")
DefNode(CompositorNode, CMP_NODE_SEPRGBA_LEGACY,    0,                      "SEPRGBA",         SepRGBA,          "Separate RGBA",       "Deprecated")
DefNode(CompositorNode, CMP_NODE_SEPYCCA_LEGACY, def_cmp_ycc,               "SEPYCCA",         SepYCCA,          "Separate YCbCrA",     "Deprecated")
DefNode(CompositorNode, CMP_NODE_SEPYUVA_LEGACY, 0,                         "SEPYUVA",         SepYUVA,          "Separate YUVA",       "Deprecated")
DefNode(CompositorNode, CMP_NODE_SETALPHA,          def_cmp_set_alpha,      "SETALPHA",        SetAlpha,         "Set Alpha",           "Add an alpha channel to an image")
DefNode(CompositorNode, CMP_NODE_SPLIT,          def_cmp_split,             "SPLIT",           Split,            "Split",               "Combine two images for side-by-side display. Typically used in combination with a Viewer node")
DefNode(CompositorNode, CMP_NODE_STABILIZE2D,    def_cmp_stabilize2d,       "STABILIZE2D",     Stabilize,        "Stabilize 2D",        "Stabilize footage using 2D stabilization motion tracking settings")
...
DefNode(CompositorNode, CMP_NODE_SWITCH_VIEW,    def_cmp_switch_view,       "VIEWSWITCH",      SwitchView,       "Switch View",         "Combine the views (left and right) into a single stereo 3D output")
DefNode(CompositorNode, CMP_NODE_SWITCH,         def_cmp_switch,            "SWITCH",          Switch,           "Switch",              "Switch between two images using a checkbox")
DefNode(CompositorNode, CMP_NODE_TEXTURE,        def_texture,               "TEXTURE",         Texture,          "Texture",             "Generate texture pattern from texture datablock")
DefNode(CompositorNode, CMP_NODE_TIME,              def_time,               "TIME",            Time,             "Time Curve",          "Generate a factor value (from 0.0 to 1.0) between scene start and end time, using a curve mapping")
DefNode(CompositorNode, CMP_NODE_TONEMAP,        def_cmp_tonemap,           "TONEMAP",         Tonemap,          "Tonemap",             "Map one set of colors to another in order to approximate the appearance of high dynamic range")
...
DefNode(CompositorNode, CMP_NODE_TRANSFORM,      dev_cmd_transform,         "TRANSFORM",       Transform,        "Transform",           "Scale, translate and rotate an image")
DefNode(CompositorNode, CMP_NODE_TRANSLATE,      def_cmp_translate,         "TRANSLATE",       Translate,        "Translate",           "Offset an image")
DefNode(CompositorNode, CMP_NODE_VALTORGB,          def_colorramp,          "VALTORGB",        ValToRGB,         "Color Ramp",          "Map values to colors with the use of a gradient")
DefNode(CompositorNode, CMP_NODE_VALUE,             0,                      "VALUE",           Value,            "Value",               "Input numerical values to other nodes in the node graph")
DefNode(CompositorNode, CMP_NODE_VECBLUR,           def_cmp_vector_blur,    "VECBLUR",         VecBlur,          "Vector Blur",         "Uses the vector speed render pass to blur the image pixels in 2D")
DefNode(CompositorNode, CMP_NODE_VIEW_LEVELS,    def_cmp_levels,            "LEVELS",          Levels,           "Levels",              "Compute average and standard deviation of pixel values")
...
DefNode(CompositorNode, CMP_NODE_ZCOMBINE,       def_cmp_zcombine,          "ZCOMBINE",        Zcombine,         "Z Combine",           "Combine two images using depth maps")
...
DefNode(GeometryNode, GEO_NODE_INPUT_INSTANCE_TRANSFORM, 0, "INPUT_INSTANCE_TRANSFORM", InstanceTransform, "Instance Transform", "Retrieve the full transformation of each instance in the geometry")
...
DefNode(GeometryNode, GEO_NODE_INPUT_MESH_EDGE_ANGLE, 0, "MESH_EDGE_ANGLE", InputMeshEdgeAngle, "Edge Angle", "The angle between the normals of connected manifold faces")
...
DefNode(GeometryNode, GEO_NODE_VORONOI, def_geo_voronoi, "VORONOI", Voronoi, "Voronoi", "Generate Voronoi Diagram")
